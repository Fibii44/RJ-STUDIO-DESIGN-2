<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 py-4">
            ...
            <a href="{{ route('services') }}" class="inline-flex items-center px-8 py-3.5 bg-slate-900 text-white rounded-xl font-bold text-sm hover:scale-105 transition shadow-xl">
                New Request
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-6">
                @forelse($appointments as $appointment)
                    <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                            <div class="flex items-start gap-6">
                                <div class="w-16 h-16 bg-sky-50 flex flex-col items-center justify-center rounded-2xl border border-sky-100 shrink-0">
                                    <span class="text-[10px] font-bold text-sky-600 uppercase">{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('M') }}</span>
                                    <span class="text-2xl font-bold text-sky-900 leading-none">{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('d') }}</span>
                                </div>
                                <div>
                                    <h3 class="text-xl font-serif text-slate-900">{{ $appointment->service_type }}</h3>
                            ...
                            <div class="flex-1 lg:max-w-md p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Project Brief</p>
                                <p class="text-xs text-slate-600 italic line-clamp-2">"{{ $appointment->message ?? 'No additional details provided.' }}"</p>
                            </div>
                        </div>
                    </div>
                @empty
