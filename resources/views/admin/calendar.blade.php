<x-admin-layout>
    <div class="space-y-8" x-data="calendar()">
        <!-- Header -->
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h3 class="text-3xl font-serif text-slate-900 leading-tight">Schedule <span class="text-sky-600 italic">Calendar</span></h3>
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1">Timeline & Appointment Pins • RJ Studio</p>
            </div>
            
            <div class="flex items-center gap-3">
                <div class="flex p-1 bg-white rounded-inner border border-slate-100 shadow-sm">
                    <button @click="prevMonth()" class="p-2 hover:bg-slate-50 rounded-btn transition-colors text-slate-400 hover:text-slate-900">
                    ...
                    <button @click="nextMonth()" class="p-2 hover:bg-slate-50 rounded-btn transition-colors text-slate-400 hover:text-slate-900">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </button>
                </div>

                <button @click="goToToday()" class="px-6 py-3 bg-slate-900 text-white rounded-btn text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
                    Today
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 bg-white rounded-card border border-slate-100 shadow-sm overflow-hidden p-8">
                ...
                <div class="grid grid-cols-7 gap-2">
                    ...
                    <template x-for="date in daysInMonth">
                        <div class="aspect-square relative group rounded-inner border transition-all duration-300 flex flex-col p-3"
                             :class="{
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white rounded-[2rem] p-6 border border-slate-100" x-show="appointmentsForSelectedDate.length > 0">
            ...
            <div class="p-8" x-show="viewingApp">
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        <div class="w-12 h-12 bg-sky-900 text-white rounded-xl flex items-center justify-center font-bold" x-text="viewingApp?.first_name[0] + viewingApp?.last_name[0]"></div>
                        <div>
                            <p class="text-xs font-black text-slate-900 uppercase tracking-widest" x-text="viewingApp?.first_name + ' ' + viewingApp?.last_name"></p>
                            <p class="text-[10px] text-slate-500 font-medium" x-text="viewingApp?.email"></p>
                        </div>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 space-y-3" x-show="viewingApp && viewingApp.status === 'pending'">
                        <div class="flex gap-4">
                            <!-- Cancel Form (Decline) -->
                            <form :action="`/appointments/${viewingApp.id}/cancel`" method="POST" class="flex-1" id="calendar-decline-form">
                                @csrf
                                @method('PATCH')
                                <button type="button" 
                                        :disabled="submitting"
                                        @click="window.dispatchEvent(new CustomEvent('open-confirm', { 
                                            detail: { 
                                                ...
                                                action: () => { submitting = true; document.getElementById('calendar-decline-form').submit(); }
                                            } 
                                        }))"
                            ...
                            <!-- Approve Form -->
                            <form :action="`/admin/appointments/${viewingApp.id}/confirm`" method="POST" class="flex-1" id="calendar-approve-form">
                                @csrf
                                @method('PATCH')
                                <button type="button" 
                                        :disabled="submitting"
                                        @click="window.dispatchEvent(new CustomEvent('open-confirm', { 
                                            detail: { 
                                                title: 'Approve Pin',
                                                message: 'Confirm this appointment? The client will be notified.',
                                                confirmButton: 'Confirm',
                                                action: () => { submitting = true; document.getElementById('calendar-approve-form').submit(); }
                                            } 
                                        }))"
