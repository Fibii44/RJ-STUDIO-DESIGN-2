<x-app-layout>
    <div class="py-24 max-w-4xl mx-auto px-6" x-data="{ 
        ...
        <div class="text-center mb-12">
            ...
            <h2 class="font-serif text-5xl mt-2 text-slate-900">Project <span class="text-sky-600 italic">Details</span></h2>
            <p class="text-slate-500 mt-4">Discussing: <strong class="text-slate-900">{{ request('service') }}</strong></p>
        </div>

        <form @submit.prevent="showModal = true" id="appointmentForm" action="{{ route('appointments.store') }}" method="POST" 
              class="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-2xl">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                <div class="space-y-2">
                    ...
                    <input type="text" name="first_name" x-model="formData.first_name" required 
                           class="w-full rounded-2xl border-slate-100 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 shadow-sm transition-all">
                </div>
                <div class="space-y-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100">
        
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div class="flex items-center justify-between mb-6">
                <h4 class="font-serif text-lg text-slate-900" x-text="monthNames[month] + ' ' + year"></h4>
                <div class="flex gap-2">
                    <button type="button" @click="prevMonth()" class="p-2 hover:bg-slate-100 rounded-full transition">&larr;</button>
                    <button type="button" @click="nextMonth()" class="p-2 hover:bg-slate-100 rounded-full transition">&rarr;</button>
                </div>
            </div>
                ...
                <template x-for="date in no_of_days">
                    <button type="button" 
                            ...
                            :class="{
                                ...
                                'hover:bg-sky-50 text-slate-700': !isSelected(date) && !isPast(date),
            ...
            <div class="grid grid-cols-2 gap-3">
                <template x-for="time in timeSlots">
                    <button type="button" 
                            ...
                            :class="formData.time === time ? 'bg-sky-600 border-sky-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-600 hover:border-sky-300'">
            ...
            <input type="hidden" name="appointment_date" :value="formData.date + ' ' + formData.time">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="message" x-model="formData.message" rows="4" placeholder="Briefly describe your style, budget, or specific requirements..."
                              class="w-full rounded-2xl border-slate-100 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 shadow-sm transition-all"></textarea>
                </div>
            </div>

            <button type="submit" class="mt-12 w-full py-5 bg-slate-900 text-white rounded-[2rem] font-bold uppercase tracking-widest hover:scale-[1.02] transition-all shadow-2xl shadow-slate-200">
        ...
        <div x-show="showModal" x-cloak 
            ...
            <div @click.away="showModal = false" class="bg-white w-full max-w-xl rounded-[3rem] p-8 md:p-12 shadow-2xl overflow-hidden relative">
                <div class="absolute top-0 left-0 w-full h-2 bg-sky-600"></div>
                
                <h3 class="font-serif text-3xl mb-2 text-slate-900 text-center">Confirm <span class="text-sky-600 italic">Brief</span></h3>
                <p class="text-center text-slate-500 text-sm mb-8">Ensure all architectural requirements are correct.</p>
                
                <div class="space-y-4 mb-10">
                    <div class="flex flex-col border-b border-slate-50 pb-3">
                        <span class="text-slate-400 uppercase text-[9px] font-black tracking-widest mb-1">Project Client</span>
                        <span class="font-bold text-slate-800" x-text="formData.first_name + ' ' + formData.last_name"></span>
                    </div>
                    <div class="flex flex-col border-b border-slate-50 pb-3">
                    ...
                    <div class="flex flex-col border-b border-slate-50 pb-3">
                        ...
                        <span class="font-bold text-slate-800" x-text="formData.location || 'Consultation Only'"></span>
                    </div>
                    <div class="flex flex-col border-b border-slate-50 pb-3">
                        <span class="text-slate-400 uppercase text-[9px] font-black tracking-widest mb-1">Meeting Schedule</span>
                        <span class="font-bold text-slate-800" x-text="formatDate(formData.date)"></span>
                    </div>
                </div>

                <div class="flex flex-col sm:flex-row gap-4">
                    <button @click="showModal = false" type="button" class="flex-1 py-4 border border-slate-200 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-50 transition">
                        Back to Edit
                    </button>
                    <button @click="document.getElementById('appointmentForm').submit()" type="button" class="flex-1 py-4 bg-sky-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-sky-200 hover:bg-slate-900 transition">
                        Confirm & Send Brief
                    </button>
                </div>
            </div>
